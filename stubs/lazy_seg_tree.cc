...
Node allNodes[Size];
int initialVals[biggestRange+1]; // inclusive range [0, biggestRange] so we plus one

void BuildTree(int index, int nodeStart, int nodeEnd) {
    if (nodeStart == nodeEnd) {
        allNodes[index].maxHeight = allNodes[index].minHeight = initialVals[nodeStart];
        return;
    }
    
    int middle = (nodeStart + nodeEnd) / 2;

    BuildTree(index * 2, nodeStart, middle);
    BuildTree(index * 2 + 1, middle + 1, nodeEnd);
